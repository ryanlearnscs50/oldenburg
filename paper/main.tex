...
near the 80th percentile of the distribution, and demarcates a range where
digit-triplet intelligibility reliably deteriorates in real-world conditions.
The cut-off was selected \emph{a priori} based on the score distribution
...
ages 70--80) is restricted to older adults, and the NHANES audiogram covers
only 500--4\,kHz.  The validation targets (AUQ054 and AUQ101) are single
self-report Likert items rather than administered speech-in-noise tests;
self-report and performance measures tap partly distinct constructs (perceived
handicap vs.\ signal detection), so cross-cohort AUROCs on these items are
not directly comparable to within-cohort AUROC on administered SIN.
