...
validation on NHANES 2017--18 ($N=859$, ages 70--80) achieves AUROC = 0.80
...
\begin{enumerate}
  ...
  \item Partial cross-cohort validation: OHHR-trained models transfer to NHANES,
        corroborating that age improves audiometric hearing difficulty prediction
        beyond the pure-tone audiogram.
...
The primary validation outcome was AUQ054 (\textit{``How often do you have
difficulty hearing?''}, 5-point scale), dichotomised as $\geq 3$ (sometimes
or more) vs.\ $<3$ (47.8\% positive rate).  A secondary outcome was AUQ101
(\textit{``Difficulty hearing in noise?''}) dichotomised at $\geq 4$.
Classifiers trained on OHHR were applied without retraining to the NHANES
sample.
...
SRT\,${>}\,-2$\,dB~SNR (21.0\% prevalence, $n=122$), a clinically
meaningful threshold indicating substantially impaired speech-in-noise
understanding.
...
This indicates that the models produce reliable probabilistic risk
estimates---a prerequisite for clinical screening use.
...
results.  A PTA4-only model trained on OHHR achieves AUROC = 0.747 on
...
raises the NHANES AUROC to 0.801 ($\Delta = +0.054$), corroborating in an
...
achieves the highest NHANES AUROC of 0.833.  Transfer to the NHANES AUQ101
speech-in-noise item yields near-chance AUROC ($\approx 0.25$--$0.30$),
reflecting the psychometric mismatch between a single binary question and
a standardised adaptive DTT SRT; cross-cohort conclusions are therefore
restricted to the general hearing difficulty endpoint.
...
\begin{tabular}{lcc}
\toprule
Feature set & OHHR AUROC & NHANES AUROC \\
            & (OOF)      & (hearing diff.) \\
\midrule
PTA4 only              & 0.943 & 0.747 \\
PTA4 + Age             & 0.968 & 0.801 \\
PTA4 + Age + Sex       & 0.968 & 0.797 \\
Matched HTL + Age      & 0.958 & \textbf{0.833} \\
\bottomrule
\end{tabular}
\end{table}
...
\textit{Cross-cohort generalisability.}  The NHANES transfer results (AUROC
0.75--0.83) are encouraging: despite a distributional shift between a German
clinical cohort and a US general population survey, PTA-derived risk scores
retain meaningful discriminative power for self-reported hearing difficulty.
The finding that age improves NHANES AUROC by $+0.054$ replicates, in an
independent population-based sample of 859 older adults, the
SHAP-identified importance of age beyond the audiogram.  The failure to
transfer to the NHANES AUQ101 speech-in-noise questionnaire item is
unsurprising given the psychometric mismatch between a single Likert item
and a standardised adaptive procedure; a more direct cross-cohort test
would require administered SIN measurements (e.g., UK Biobank digit-triplet
data or the LISTEN corpus).
...
cross-cohort AUROC from 0.747 to 0.801.  An MLP neural baseline is
...
\textit{Scientific Data}, vol.~12, 2025, Art. no. 705.
...
\begin{figure*}[t]
  ...
  by feature set; adding age to PTA4 raises AUROC from 0.747 to 0.801.
  (b) Distribution of model-predicted P(poor SIN) stratified by NHANES
  AUQ054 category (1 = never, 5 = almost always); predicted risk increases
  monotonically with reported difficulty. (c) Calibration of PTA4-only and
