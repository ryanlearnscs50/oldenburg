...
self-reported hearing difficulty (AUROC: 0.747 $\to$ 0.801).  All code is
released openly.
\end{abstract}
...
cross-cohort validation on NHANES confirming that age improves audiometric
hearing difficulty prediction in an independent general-population sample.
...
validation outcome was AUQ054 (\textit{``How often do you have difficulty
hearing?''}, 5-point scale), dichotomised as $\geq$3 (sometimes or more;
positive rate 47.8\%).  A secondary outcome was AUQ101 (\textit{``Difficulty
hearing in noise?''}) dichotomised at $\geq$4.  Classifiers trained on
OHHR were applied without retraining to the NHANES sample.
...
meaningful threshold indicating substantially impaired SIN understanding.
...
predicted probabilities and observed event rates, indicating reliable
...
AUROC\,=\,0.747 for predicting NHANES self-reported hearing difficulty---above
...
to 0.801 ($\Delta=+0.054$), corroborating in an independent sample that age
is an informative predictor beyond threshold sensitivity.  The matched
8-frequency HTL model achieves the highest NHANES AUROC of 0.833.  Transfer
to the NHANES AUQ101 SIN questionnaire item yields near-chance AUROC
($\approx0.25$--$0.30$), attributable to the psychometric mismatch between a
single questionnaire item and a standardised adaptive DTT.

\begin{table}[htbp]
\caption{\textbf{Cross-cohort transfer: OHHR $\to$ NHANES ($N=859$, ages 70--80).}
OHHR AUROC is out-of-fold; NHANES AUROC predicts dichotomised AUQ054 hearing
difficulty.}
...
\begin{tabular}{lcc}
\toprule
Feature set & OHHR AUROC (OOF) & NHANES AUROC \\
\midrule
...
(AUROC 0.75--0.83) confirm that OHHR-trained models retain meaningful
discriminative power for self-reported hearing difficulty in a fully
independent general-population sample.  Adding age improves NHANES AUROC
by $+0.054$, replicating the SHAP finding.  The failure to transfer to the
NHANES AUQ101 SIN item reflects the psychometric mismatch between a single
Likert item and a standardised adaptive procedure.
...
\url{[repository URL upon acceptance]} under an open-source licence.
...
\textit{Scientific Data} \textbf{12}, 705 (2025).
...
\begin{figure*}[htbp]
  ...
  (AUQ054) by feature set.  Adding age to PTA4 raises AUROC from 0.747 to
  0.801.
  \textbf{(b)}~Distribution of predicted P(poor SIN) by NHANES AUQ054
  category (1\,=\,never, 5\,=\,almost always); predicted risk increases
  monotonically.
