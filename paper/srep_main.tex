...
AUROC\,=\,0.978 (Brier\,=\,0.053).  Partial cross-cohort construct validation on NHANES
...
approximately 2.6\,dB above the cohort mean ($-4.6$\,dB~SNR, $SD=4.2$\,dB),
placing it near the 80th percentile and demarcating a clinically relevant
range where digit-triplet intelligibility reliably deteriorates under
real-world listening conditions.  The threshold was selected \emph{a priori}
based on the DTT score distribution; we report results at this fixed cut-off
without post-hoc optimisation.
...
older adults, and NHANES audiometry covers only 500--4\,kHz.  Crucially,
the validation targets (AUQ054 and AUQ101) are single self-report Likert
items, not administered speech-in-noise tests; self-report and audiometric
performance tap partly distinct constructs (perceived handicap vs.\ objective
signal detection), so AUROC values on these items cannot be compared directly
to AUROC values on administered SIN endpoints.  The moderate-loss
